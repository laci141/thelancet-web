...
#
# thelancet-web: Go wrapper serving the UI + /affiliations + /authors endpoints
# against a local mirror DB. The mirror (data.db, ~117 MB, full Lancet history
# 2005-2026, 35k works) is stored via Git LFS. Because Render's Docker build does
# NOT auto-fetch LFS objects, an LFS-aware stage pulls the real DB explicitly.
